...
# 경고 후 실행 중단
if not codes:
    st.warning('중복 코드를 찾을 수 없습니다. 종목을 다시 선택해주세요.')
    st.stop()
...
def get_stock_data(code, start, end):
    '''
    함수설명 : FinaceDataReader로 개별 종목의 주가 데이터 조회
...
with tab1:
    if len(codes) == 1: # 종목을 하나만 선택했다면
        ...
        if dfs: # 병합할 데이터가 있다면
            ...
            merged_df.columns = selected_stocks # 열이름을 종목명(한글)로
