...
if uploaded_confirmed and uploaded_deaths and uploaded_recovered:
    ...
    df_recoverd = pd.read_csv(uploaded_recovered)
    ...
    df_recoverd = get_korea_data(df_recoverd, 'recovered') # 회복자

    # 7. 세 개의 데이터프레임을 하나로 병합 (merge)
    #   'date' 열을 기준으로 병합 --> on='date
    df_merged = df_confirmed.merge(df_deaths, on='date').merge(df_recoverd, on='date')
    ...
    df_merged['new_deaths'] = df_deaths['deaths'].diff().fillna(0).astype(int)
